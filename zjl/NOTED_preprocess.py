...
class Preprocessor: #Preprocessor类负责对单细胞数据进行预处理，包括过滤、标准化、分箱（binning）等操作。
    """
    ...
    def __init__(
        self,
        filter_gene_by_counts: Union[int, bool] = False, #根据基因表达量过滤基因。
        filter_cell_by_counts: Union[int, bool] = False, #根据细胞表达量过滤细胞。
        normalize_sum: float = 1e4, #对细胞的总表达量进行标准化。
        n_hvg_for_postp: int = 0,
        use_layer: Optional[str] = None,
        is_symbol: bool = False,
        hvg_flavor: str = "seurat_v3", #选择高变基因的方法（如seurat_v3）
        binning: Optional[int] = None, #将表达量分箱为离散值。
        ...
        additional_preprocess: Optional[Callable[[AnnData], AnnData]] = None, #用户自定义的预处理和后处理函数。
    ...
    def __call__(self, adata) -> AnnData:
        """调用方法 (__call__)
        对输入的AnnData对象执行一系列预处理步骤：
        数据检查：检查数据是否为原始计数数据（raw counts）。
        过滤：根据基因和细胞的表达量过滤数据。
        标准化：对细胞的总表达量进行标准化。
        高变基因选择：选择高变基因用于后续分析。
        分箱：将表达量分箱为离散值（如果启用了binning）。
        质量控制（QC）：计算质量控制指标，如线粒体基因比例等。
        后处理：执行用户自定义的后处理函数（如果提供了）。
        """
        ...
        ).sum():  # check if likely raw data  检查是否为原始数据
        ...
        # # cleanup and dropping low expressed genes and unexpressed cells  数据清洗
        prevsize = adata.shape[0]
        ...
        # load the genes
        genesdf = data_utils.load_genes(adata.obs.organism_ontology_term_id.iloc[0]) #要求有organism_ontology_term_id列
...
class LaminPreprocessor(Preprocessor): #LaminPreprocessor类继承自Preprocessor，专门用于处理lamindb中的数据集合（ln.Collection）。
    def __init__(
        self,
        *args,
        cache: bool = True,
        stream: bool = False,
        keep_files: bool = True, #增加了cache、stream和keep_files参数，用于控制数据缓存、流式加载和文件保存。
        **kwargs,
    ):
    ...
    def __call__(
        ...
        if self.cache:
            for i in ln.Artifact.filter(description=description): #处理ln.Collection中的每个Artifact（数据文件）：
        ...
        if isinstance(data, AnnData):
            ...
            for i, file in enumerate(data.artifacts.all()[start_at:]):
                ...
                if backed.shape[1] < 1000:
                    ...
                    continue
                if file.size <= MAXFILESIZE: #数据加载：根据文件大小决定是否分块加载。
                ...
                try:
                    if file.size > MAXFILESIZE:
                        ...
                        adata = super().__call__(adata) #预处理：调用父类的__call__方法对数据进行预处理。
                        try:
            ...
            if self.keep_files:
                dataset = ln.Collection(files, name=name, description=description) #保存结果：将预处理后的数据保存为新的Artifact，并可选地保存到ln.Collection中。
                dataset.save()
                return dataset
            else:
                return
...
def is_log1p(adata: AnnData) -> bool:
    """
    Check if the data is already log1p transformed. 工具函数is_log1p：检查数据是否已经过log1p转换。
...
def _digitize(x: np.ndarray, bins: np.ndarray, side="both") -> np.ndarray:
    ...
    have same values. 工具函数：_digitize：将数据分箱为离散值。
...
def binning(row: np.ndarray, n_bins: int) -> np.ndarray:
    """Binning the row into n_bins.工具函数：binning：对单行数据进行分箱。"""
...
def additional_preprocess(adata): #additional_preprocess：用户自定义的预处理函数，用于处理细胞类型和组织类型的元数据。
    ...
    )  # multi ethnic will have to get renamed 将多民族、美国、犹太以色列等分类标记为unknown。
    adata.obs["cell_culture"] = False
    # if cell_type contains the word "(cell culture)" then it is a cell culture and we mark it as so and remove this from the cell type 标记细胞培养和组织培养的样本。
...
def additional_postprocess(adata): #用户自定义的后处理函数，用于执行高级分析（如palantir的扩散映射和伪时间分析）。
    import palantir
